angular.module('page', ["ideUI", "ideView", "entityApi"])
	...
	.controller('PageController', ['$scope', 'messageHub', 'ViewParameters', 'entityApi', function ($scope, messageHub, ViewParameters, entityApi) {
		...
		if (Object.keys(params).length) {
			...
			$scope.optionsNormal = params.optionsNormal;
		}
		...
		$scope.create = function () {
			...
			entityApi.create(entity).then(function (response) {
				if (response.status != 201) {
					$scope.errorMessage = `Unable to create Account: '${response.message}'`;
					return;
				}
		...
		$scope.update = function () {
			...
			entityApi.update(id, entity).then(function (response) {
				if (response.status != 200) {
					$scope.errorMessage = `Unable to update Account: '${response.message}'`;
					return;
				}
		...
		$scope.cancel = function () {
			$scope.entity = {};
			$scope.action = 'select';
			messageHub.closeDialogWindow("Account-details");
		};
